...
public class OSGlideLoader {
    ...
    public static void loadUserProfileImage(Context context, String uid, ImageView imageView) {
        OSImageLoader.getUserProfileImage(context, uid, (OnStringResponse) url -> Glide.with(context).load(url).listener(new RequestListener<Drawable>() {
            @Override
            public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean isFirstResource) {
                OSVolley.getInstance(context).removeCache(uid);
                OSImageLoader.getUserProfileImage(context, uid, (OnStringResponse) url -> Glide.with(context).load(url).circleCrop().into(imageView), null);
                return false;
            }
